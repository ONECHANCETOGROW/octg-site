...
foreach ($files as $file) {
    ...
    try {
        ...
        // Ignore duplicate tables/columns/keys or drop errors on repeat runs
        $ignoredErrors = ['1050', '1060', '1061', '1091', '42S21', '42S02'];
        $isIgnored = false;
        foreach ($ignoredErrors as $err) {
            if (strpos($e->getMessage(), $err) !== false) {
                $isIgnored = true;
                break;
            }
        }
        ...
        if ($isIgnored) {
            echo "Already applied (Ignored duplicate/missing columns).\n";
        } else {
