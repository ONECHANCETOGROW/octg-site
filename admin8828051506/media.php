...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_FILES['media_upload'])) {
    if (!octg_verify_csrf()) {
        ...
        if ($file['error'] === UPLOAD_ERR_OK) {
            ...
            $allowed = ['jpg', 'jpeg', 'png', 'svg', 'webp', 'gif', 'mp4', 'webm', 'avif'];
            if (in_array($ext, $allowed)) {
                ...
                if (move_uploaded_file($file['tmp_name'], $targetPath)) {
                    ...
                    try {
                        ...
                        // If columns don't exist, try fallback insert without mime_type and file_size
                        if (strpos($e->getMessage(), "Unknown column 'mime_type'") !== false) {
                            try {
                                $stmt = $pdo->prepare('INSERT INTO cms_media (file_path, title, alt_text, category) VALUES (?, ?, ?, ?)');
                                $stmt->execute([$webPath, $title, $alt_text, $category]);
                                $success = 'Media uploaded (legacy schema mode).';
                            } catch (PDOException $e2) {
                                $error = 'Database error (legacy): ' . $e2->getMessage();
                            }
                        } else {
    ...
    <div class="admin-card__body">
      <form method="post" action="media.php" enctype="multipart/form-data" class="admin-form">
        ...
        <div class="admin-form__row">
          ...
          <input type="file" name="media_upload" required accept="image/*,video/*">
        </div>
          ...
          <div class="media-item" style="border:1px solid #E2E1D9; border-radius:4px; padding:10px; background:#fff;">
            <div style="height:140px; background:#F7F6F1; display:flex; align-items:center; justify-content:center; margin-bottom:10px; overflow:hidden; border-radius:3px;">
                <?php if (strpos($item['mime_type'] ?? '', 'video/') === 0): ?>
                    <video src="<?php echo htmlspecialchars($item['file_path']); ?>" style="max-width:100%; max-height:100%; object-fit:contain;" muted loop autoplay playsinline></video>
                <?php else: ?>
