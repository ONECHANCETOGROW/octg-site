...
$id = (int) ($_GET['id'] ?? 0);
if (!$pdo || !$id) { header('Location: /admin8828051506/leads.php'); exit; }

$stmt = $pdo->prepare('SELECT * FROM leads WHERE id = :id');
$stmt->execute([':id' => $id]);
$lead = $stmt->fetch();
if (!$lead) { header('Location: /admin8828051506/leads.php'); exit; }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'update_status') {
        ...
        $pdo->prepare('UPDATE leads SET status = :s, follow_up_date = :f, assigned_to = :a WHERE id = :id')
            ->execute([':s' => $newStatus, ':f' => $followUp, ':a' => $assignedTo, ':id' => $id]);
        ...
        octg_log_activity('Lead deleted', "Lead #{$id} ({$lead['name']})");
        $pdo->prepare('DELETE FROM leads WHERE id = :id')->execute([':id' => $id]);
        header('Location: /admin8828051506/leads.php');
        exit;
    }
    header('Location: /admin8828051506/lead-detail.php?id=' . $id);
    exit;
}
...
/* Assignable admin users — the leads table already had an assigned_to FK
   (sql/006_leads.sql) but no UI ever read or wrote it. */
$assignableUsers = $pdo->query('SELECT id, name FROM admin_users WHERE is_active = 1 ORDER BY name')->fetchAll();
$assignedUserName = null;
foreach ($assignableUsers as $u) {
    if ((int) $u['id'] === (int) ($lead['assigned_to'] ?? 0)) { $assignedUserName = $u['name']; break; }
}
...
<div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:20px;">
  <a href="/admin8828051506/leads.php" class="admin-btn admin-btn-small">&larr; All Leads</a>
  <form method="POST" onsubmit="return confirm('Permanently delete this lead? This cannot be undone.');">
    <input type="hidden" name="action" value="delete_lead">
    <button type="submit" class="admin-btn admin-btn-small admin-btn--danger">Delete Lead</button>
  </form>
</div>
  ...
  <div class="admin-panel">
    ...
    <div class="admin-panel__body">
      <table class="admin-table">
        ...
        <tr><td><strong>Phone</strong></td><td><?php echo $lead['phone'] ? '<a href="tel:' . htmlspecialchars($lead['phone']) . '">' . htmlspecialchars($lead['phone']) . '</a>' : '—'; ?></td></tr>
        ...
        <tr><td><strong>Assigned To</strong></td><td><?php echo htmlspecialchars($assignedUserName ?? 'Unassigned'); ?></td></tr>
        ...
        <form method="POST">
          ...
          <div class="admin-field">
            ...
            <select name="assigned_to">
              ...
              <option value="<?php echo $u['id']; ?>" <?php echo (int) $u['id'] === (int) ($lead['assigned_to'] ?? 0) ? 'selected' : ''; ?>><?php echo htmlspecialchars($u['name']); ?></option>
              <?php endforeach; ?>
            </select>
          </div>
